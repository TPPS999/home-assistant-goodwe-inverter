...
_HCA_NUMBERS = (
    ...
    GoodweNumberEntityDescription(
        key="household_breaker",
        translation_key="household_breaker",
        entity_category=EntityCategory.CONFIG,
        device_class=NumberDeviceClass.CURRENT,
        native_unit_of_measurement=UnitOfElectricCurrent.AMPERE,
        native_step=1,
        native_min_value=0,
        native_max_value=200,
        getter=lambda inv: inv.read_setting("household_breaker"),
        mapper=lambda v: int(v) if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("household_breaker", int(val)),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="max_capacity",
        ...
        getter=lambda inv: inv.read_setting("max_capacity"),
        mapper=lambda v: v if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("max_capacity", val),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="min_capacity",
        translation_key="min_capacity",
        entity_category=EntityCategory.CONFIG,
        device_class=NumberDeviceClass.ENERGY,
        native_unit_of_measurement=UnitOfEnergy.KILO_WATT_HOUR,
        native_step=0.1,
        native_min_value=0,
        native_max_value=200.0,
        getter=lambda inv: inv.read_setting("min_capacity"),
        mapper=lambda v: v if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("min_capacity", val),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="battery_discharge_soc_set",
        translation_key="battery_discharge_soc_set",
        entity_category=EntityCategory.CONFIG,
        ...
        native_min_value=0,
        native_max_value=100,
        getter=lambda inv: inv.read_setting("battery_discharge_soc_set"),
        mapper=lambda v: int(v) if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("battery_discharge_soc_set", int(val)),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="completion_time_set",
        translation_key="completion_time_set",
        entity_category=EntityCategory.CONFIG,
        ...
        native_min_value=0,
        native_max_value=10,
        getter=lambda inv: inv.read_setting("completion_time_set"),
        mapper=lambda v: int(v) if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("completion_time_set", int(val)),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="reservation_duration_set",
        translation_key="reservation_duration_set",
        entity_category=EntityCategory.CONFIG,
        device_class=NumberDeviceClass.DURATION,
        ...
        getter=lambda inv: inv.read_setting("reservation_duration_set"),
        mapper=lambda v: int(v) if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("reservation_duration_set", int(val)),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="max_capacity_reservation_set",
        translation_key="max_capacity_reservation_set",
        entity_category=EntityCategory.CONFIG,
        device_class=NumberDeviceClass.ENERGY,
        native_unit_of_measurement=UnitOfEnergy.KILO_WATT_HOUR,
        native_step=0.1,
        native_min_value=0,
        native_max_value=200.0,
        getter=lambda inv: inv.read_setting("max_capacity_reservation_set"),
        mapper=lambda v: v if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("max_capacity_reservation_set", val),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="min_capacity_reservation_set",
        translation_key="min_capacity_reservation_set",
        entity_category=EntityCategory.CONFIG,
        device_class=NumberDeviceClass.ENERGY,
        native_unit_of_measurement=UnitOfEnergy.KILO_WATT_HOUR,
        native_step=0.1,
        native_min_value=0,
        native_max_value=200.0,
        getter=lambda inv: inv.read_setting("min_capacity_reservation_set"),
        mapper=lambda v: v if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("min_capacity_reservation_set", val),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="max_charging_power_reservation",
        translation_key="max_charging_power_reservation",
        entity_category=EntityCategory.CONFIG,
        device_class=NumberDeviceClass.POWER,
        native_unit_of_measurement=UnitOfPower.KILO_WATT,
        native_step=0.1,
        native_min_value=1.4,
        native_max_value=22.0,
        getter=lambda inv: inv.read_setting("max_charging_power_reservation"),
        mapper=lambda v: v if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("max_charging_power_reservation", val),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="battery_discharge_soc_reservation",
        translation_key="battery_discharge_soc_reservation",
        entity_category=EntityCategory.CONFIG,
        device_class=NumberDeviceClass.BATTERY,
        native_unit_of_measurement=PERCENTAGE,
        native_step=1,
        native_min_value=0,
        native_max_value=100,
        getter=lambda inv: inv.read_setting("battery_discharge_soc_reservation"),
        mapper=lambda v: int(v) if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("battery_discharge_soc_reservation", int(val)),
        filter=lambda inv: True,
    ),
    GoodweNumberEntityDescription(
        key="completion_time_reservation_set",
        translation_key="completion_time_reservation_set",
        entity_category=EntityCategory.CONFIG,
        device_class=NumberDeviceClass.DURATION,
        native_unit_of_measurement=UnitOfTime.HOURS,
        native_step=1,
        native_min_value=0,
        native_max_value=10,
        getter=lambda inv: inv.read_setting("completion_time_reservation_set"),
        mapper=lambda v: int(v) if v is not None else 0,
        setter=lambda inv, val: inv.write_setting("completion_time_reservation_set", int(val)),
        filter=lambda inv: True,
    ),
)
